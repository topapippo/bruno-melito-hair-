...
def _backup_file(user_id: str) -> str:
    """File 'ultimo backup' per il download rapido — scoped per utente."""
    return os.path.join(_BACKUP_DIR, f"salon_backup_{user_id}.json")


def _dated_backup_file(user_id: str, date_str: str) -> str:
    return os.path.join(_BACKUP_DIR, f"salon_backup_{user_id}_{date_str}.json")


def _rotate_backups(user_id: str):
    """Mantiene gli ultimi BACKUP_ROTATE_DAYS backup giornalieri di questo utente, elimina i più vecchi."""
    try:
        prefix = f"salon_backup_{user_id}_"
        files = sorted([
...
async def run_backup(user_id: str) -> bool:
    """
    Esegue il backup completo dei dati del salone di questo utente.
    Mantiene un file per data (rotante, max BACKUP_ROTATE_DAYS giorni).
    Aggiorna sempre anche il file "ultimo backup" per il download rapido.
    """
    dated_file = None
    try:
        logger.info(f"Backup avviato per user_id={user_id}...")
        ...
        dated_file = _dated_backup_file(user_id, today_str)
        ...
        # Copia anche sul file "ultimo backup" (per compatibilità con il download rapido)
        import shutil
        shutil.copy2(dated_file, _backup_file(user_id))

        # Elimina backup più vecchi di BACKUP_ROTATE_DAYS
        _rotate_backups(user_id)

        total_records = sum(len(v) for v in backup_data.values() if isinstance(v, list))
        logger.info(
            f"Backup completato [{today_str}] per user_id={user_id}: {total_records} record — "
...
async def _collect_all(user_id: str):
    """Recupera tutti i dati dal DB per questo utente in modo sequenziale."""
    q = {"user_id": user_id}
    appointments  = await db.appointments.find(q, {"_id": 0}).to_list(50_000)
    clients       = await db.clients.find(q, {"_id": 0}).to_list(20_000)
    payments      = await db.payments.find(q, {"_id": 0}).to_list(50_000)
    expenses      = await db.expenses.find(q, {"_id": 0}).to_list(20_000)
    cards         = await db.cards.find(q, {"_id": 0}).to_list(5_000)
    card_templates = await db.card_templates.find(q, {"_id": 0}).to_list(1_000)
    loyalty       = await db.loyalty.find(q, {"_id": 0}).to_list(20_000)
    loyalty_rewards = await db.loyalty_rewards.find(q, {"_id": 0}).to_list(1_000)
    operators     = await db.operators.find(q, {"_id": 0}).to_list(500)
    services      = await db.services.find(q, {"_id": 0}).to_list(500)
    promotions    = await db.promotions.find(q, {"_id": 0}).to_list(500)
    blocked_slots = await db.blocked_slots.find(q, {"_id": 0}).to_list(2_000)
    sospesi       = await db.sospesi.find(q, {"_id": 0}).to_list(20_000)
    # Utente: escludi il campo password per sicurezza
    own_user      = await db.users.find_one({"id": user_id}, {"_id": 0, "password": 0})
...
async def backup_status(current_user: dict = Depends(get_current_user)):
    ...
    # Lista tutti i backup giornalieri disponibili di questo utente
    available = []
    try:
        prefix = f"salon_backup_{uid}_"
        for fname in sorted(os.listdir(_BACKUP_DIR), reverse=True):
            if fname.startswith(prefix) and fname.endswith(".json"):
                fpath = os.path.join(_BACKUP_DIR, fname)
                fstat = os.stat(fpath)
                date_part = fname.replace(prefix, "").replace(".json", "")
                available.append({
                    "date": date_part,
                    "filename": fname,
...
async def download_backup_by_date(date: str, current_user: dict = Depends(get_current_user)):
    ...
    uid = current_user["id"]
    fpath = _dated_backup_file(uid, date)
...
async def download_backup_pdf(current_user: dict = Depends(get_current_user)):
    ...
    uid = current_user["id"]
    (
        appointments_u, clients_u, payments_u, expenses_u, cards_u, card_templates,
        loyalty, loyalty_rewards, operators_u, services_u, promotions,
        blocked_slots, sospesi, users,
    ) = await _collect_all(uid)
