    ...
    <>
      <section className='py-24 sm:py-32 overflow-hidden relative' style={{ background: '#020205' }}>
        ...
        <div className='max-w-7xl mx-auto px-6'>
            ...
            {hairstylePhotos.map((item, idx) => (
              ...
              >
                ...
                {item.file_type === 'video' ? (
                  ...
                  <img src={getMediaUrl(item?.image_url)} alt={item.label || ''} className='w-full object-cover transition-transform duration-1000 group-hover:scale-110' />
                )}
          ...
          >
            <button onClick={() => setShowBooking(true)} className='group relative inline-flex items-center gap-4 px-12 py-6 font-black text-white uppercase tracking-[0.2em] rounded-2xl overflow-hidden shadow-2xl'>
      ...
      {lightboxIdx !== null && lightboxIdx >= 0 && (
        <div className='fixed inset-0 z-[100] bg-black/98 backdrop-blur-2xl flex items-center justify-center p-4 cursor-zoom-out' onClick={() => setLightboxIdx(null)}>
          <button onClick={() => setLightboxIdx(null)} className='absolute top-8 right-8 w-12 h-12 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center text-white transition-colors z-10 border border-white/10'><X className='w-6 h-6' strokeWidth={1} /></button>
          <div className='relative max-w-5xl w-full flex items-center justify-center'>
            {imagePhotos.length > 1 && (
              <>
                <button onClick={(e) => { e.stopPropagation(); setLightboxIdx(p => p > 0 ? p - 1 : imagePhotos.length - 1); }} className='absolute -left-4 sm:left-4 w-12 h-12 rounded-full bg-white/5 hover:bg-white/10 flex items-center justify-center text-white transition-colors z-10 border border-white/5 backdrop-blur'><ArrowLeft className='w-6 h-6' /></button>
                <button onClick={(e) => { e.stopPropagation(); setLightboxIdx(p => p < imagePhotos.length - 1 ? p + 1 : 0); }} className='absolute -right-4 sm:right-4 w-12 h-12 rounded-full bg-white/5 hover:bg-white/10 flex items-center justify-center text-white transition-colors z-10 border border-white/5 backdrop-blur'><ArrowRight className='w-6 h-6' /></button>
              </>
            )}
            <motion.div key={lightboxIdx} initial={{ scale: 0.9, opacity: 0, filter: 'blur(10px)' }} animate={{ scale: 1, opacity: 1, filter: 'blur(0px)' }} transition={{ duration: 0.4, ease: _SITE_EASE }} className='relative rounded-[3rem] overflow-hidden shadow-[0_0_80px_rgba(168,85,247,0.3)] border border-white/10'>
