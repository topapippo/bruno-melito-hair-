...
async def sum_payments(uid: str, start_date: str, end_date: str) -> float:
    """Somma total_paid di tutti i pagamenti dell'utente nell'intervallo di date usando Aggregation."""
    pipeline = [
...
async def get_dashboard_stats(current_user: dict = Depends(get_current_user)):
    uid = current_user["id"]
    # FIX FUSO ORARIO: Aggiungiamo 2 ore per allinearci all'ora italiana (CEST)
    now_it = datetime.now(timezone.utc) + timedelta(hours=2)
    today = now_it.strftime("%Y-%m-%d")
    first_of_month = now_it.replace(day=1).strftime("%Y-%m-%d")
    last_of_month = ((now_it.replace(day=28) + timedelta(days=4)).replace(day=1) - timedelta(days=1)).strftime("%Y-%m-%d")
    first_of_year = now_it.replace(month=1, day=1).strftime("%Y-%m-%d")
    last_of_year = now_it.replace(month=12, day=31).strftime("%Y-%m-%d")
    next_week = (now_it + timedelta(days=7)).strftime("%Y-%m-%d")
    ...
    now_time = now_it.strftime("%H:%M")
    in_2h_time = (now_it + timedelta(hours=2)).strftime("%H:%M")
    next_2h = [
    ...
    return {
        ...
        "monthly_revenue": monthly_revenue, "monthly_appointments": monthly_appointments,
        "monthly_target": current_user.get("monthly_target", 0) or 0,
        "yearly_revenue": yearly_revenue, "yearly_appointments": yearly_appointments,
...
async def get_weekly_earnings(current_user: dict = Depends(get_current_user)):
    now_it = datetime.now(timezone.utc) + timedelta(hours=2)
    today = now_it.strftime("%Y-%m-%d")
    week_ago = (now_it - timedelta(days=6)).strftime("%Y-%m-%d")
    payments = await db.payments.find(
    ...
    for i in range(6, -1, -1):
        d = (now_it - timedelta(days=i)).strftime("%Y-%m-%d")
...
async def get_morning_briefing(current_user: dict = Depends(get_current_user)):
    ...
    now_it = datetime.now(timezone.utc) + timedelta(hours=2)
    today = now_it.strftime("%Y-%m-%d")
    seven_days_later = (now_it + timedelta(days=7)).strftime("%Y-%m-%d")
